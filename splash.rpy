...
image dayonepatch:
    ...
    "white"
    0.5
    "mod-assets/day-one.png" with Dissolve(0.5, alpha=True)
    2.5
    "white" with Dissolve(0.5, alpha=True)
    0.5
...
label splashscreen:
    ...
    $ config.main_menu_music = audio.rules
    $ renpy.music.play(config.main_menu_music)
    ...
    show dayonepatch with Dissolve(0.5, alpha=True)
    $ pause(5.5 - (datetime.datetime.now() - starttime).total_seconds())
    hide dayonepatch with Dissolve(max(0, 6.0 - (datetime.datetime.now() - starttime).total_seconds()), alpha=True)
    if persistent.playthrough == 2 and renpy.random.randint(0, 3) == 0:
        $ splash_message = renpy.random.choice(splash_messages)
    show splash_warning "[splash_message]" with Dissolve(max(0, 6.5 - (datetime.datetime.now() - starttime).total_seconds()), alpha=True)
    $ pause(8.5 - (datetime.datetime.now() - starttime).total_seconds())
    hide splash_warning with Dissolve(max(0, 9.0 - (datetime.datetime.now() - starttime).total_seconds()), alpha=True)
    $ pause(9.0 - (datetime.datetime.now() - starttime).total_seconds())
    $ config.allow_skipping = True
    return
